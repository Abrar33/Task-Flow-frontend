...
export const getBoards = async () => {
  try {
    const response = await axios.get(`${API_BASE_URL}/boards`, getAuthHeaders());
...
export const addListToBoard = async (boardId, listData) => {
    try {
        const response = await axios.post(`${API_BASE_URL}/boards/${boardId}/lists`, listData, getAuthHeaders());
        return response.data;
    } catch (error) {
        throw error.response.data;
    }
...
export const inviteUserToBoard = async (boardId, email, role) => {
  try {
    const response = await axios.post(`${API_BASE_URL}/boards/${boardId}/invite`, { email, role }, getAuthHeaders());
    return response.data;
  } catch (error) {
    throw error.response.data;
  }
...
export const deleteListFromBoard = async (boardId, listId) => {
  try {
    const response = await axios.delete(
      `${API_BASE_URL}/boards/${boardId}/lists/${listId}`,
      getAuthHeaders()
    );
    return response.data;
  } catch (error) {
...
    const response = await axios.delete(
      `${API_BASE_URL}/boards/${id}/remove-user`,
      config
    );
    return response.data;
  } catch (error) {
