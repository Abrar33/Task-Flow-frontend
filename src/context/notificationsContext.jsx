...
export const NotificationProvider = ({ children }) => {
  ...
  const fetchNotifications = useCallback(async (pageToFetch = 1) => {
    ...
    try {
      const res = await fetch(`https://taskflow-backend-production-73ac.up.railway.app/api/notifications?page=${pageToFetch}&limit=20`, {
        headers: {
  ...
  const markNotificationAsRead = useCallback(async (id) => {
    ...
    try {
      const res = await fetch(`https://taskflow-backend-production-73ac.up.railway.app/api/notifications/${id}/read`, {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("authToken")}`,
        },
  ...
  const markAllAsRead = useCallback(async () => {
    // ... (rest of the code is the same)
    try {
      const res = await fetch("https://taskflow-backend-production-73ac.up.railway.app/api/notifications/readAll", {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("authToken")}`,
        },
  ...
  const deleteNotification = useCallback(async (id) => {
    // ... (rest of the code is the same)
    try {
      const res = await fetch(`https://taskflow-backend-production-73ac.up.railway.app/api/notifications/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("authToken")}`,
        },
